...
export const ActivityShow = () => {
  const { record } = useShowController({ resource: 'activities' });
  ...
  const { data, isLoading: timetrackingLoading } = useGetList(
    'rpc/timetracking_summary',
    {
      ...
      filter: {
        'axes@': `{${['activity', 'daterange', ...axesValues].join(',')}}`,
        'date_part@': granularity.value,
        'zoom_level@': '2',
        activity_id: record?.id,
        'daterange@cs': daterange.value,
      },
      meta: { columns: ['id', 'activity_id', 'daterange', 'data'] },
    },
  ...
  const handleDownload = async () => {
    ...
    const { data } = await dataProvider.getList('rpc/timetracking_summary_export', {
      ...
      meta: {
        columns: ['id', 'activity', 'daterange', ...axesValues, 'total_duration'],
      },
    });
          ...
          <Box display="flex" className="chart-container">
            <Box className="chart-left">
              ...
              <DateRangeSelect
                ...
                filter={{
                  'dims@': '{activity_id}',
                  'dimensions->>activity_id': record?.id,
                }}
                granularity={granularity}
              />
            </Box>
